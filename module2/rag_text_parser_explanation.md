# RAG Text Document Parser — Code Explanation

## What is RAG? (Quick Context)

RAG (**Retrieval-Augmented Generation**) systems work in the following steps:

1. **Ingest** documents → split them into small chunks
2. **Store** those chunks in a vector database
3. **Retrieve** relevant chunks when a user asks a question
4. **Feed** those chunks to an LLM to generate a grounded answer

> This code handles **Step 1** — parsing and chunking documents for ingestion.
...
**Why overlap?**
So that if a key sentence lands near the boundary of a chunk, it won't be completely cut off from the next chunk's context. Think of it like two puzzle pieces that slightly overlap at the edges.
...
This method reads the file and collects **metadata** — information *about* the document. This metadata travels alongside each chunk later, so you always know which file a chunk came from.
...
This is the most important and complex method. It uses a **sliding window** approach with smart boundary detection.

### Step 1 — Start with a Naive End Position
...
It begins by assuming the chunk ends exactly at `chunk_size`, then tries to be smarter about where to cut.

---

### Step 2 — Priority 1: Cut at a Sentence Boundary
...
### Step 3 — Priority 2: Cut at a Word Boundary
...
### Step 5 — Move the Window Forward (with Overlap)

```python
start = end - self.chunk_overlap if end < len(text) else end
```

Instead of jumping fully to `end`, the window steps back by `chunk_overlap` characters, so the next chunk **repeats the tail** of the current one.

```
Chunk 1:  |=========================|
Chunk 2:                     |=========================|
                             ^--- overlap zone (shared content)
```
...
raw .txt file
 ...
 parse_file()
      │  → reads content
      │  → extracts metadata (name, size, word count, etc.)
      │
      ▼
 chunk_text()
      │  → splits by chunk_size
      │  → tries sentence boundary → word boundary
      │  → slides forward with overlap
      │
      ▼
 process_document()
      ...
      │
      ▼
 List of chunks  →  ready for vector embedding & storage
```
