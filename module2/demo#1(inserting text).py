...
class TextDocumentParser:
    ...
    def parse_file(self, file_path: str) -> Dict:
        """Parse text file and extract content with metadata"""
        path = Path(file_path)
    ...
    def _generate_doc_id(slef, file_path: str) ->str:
        """Generate unique document id based on file path"""
        return hashlib.md5(file_path.encode()).hexdigest()
    
    def chunk_text(self, text: str) -> List[Dict]:
        """Split text into overlapping chunks for RAD processing"""
        chunks = []
        start = 0
        chunk_id = 0
        ...
        while start<len(text):
            ...
            if end < len(text):
                ...
                #Search backwords for sentence ending
